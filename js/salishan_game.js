    ...
    <section class="scoreboard" data-hidden="true">
      ...
      <div class="table" role="table">
        <nn-fila break="sm" class="table-header" gap=".25rem" role="row">
          <nn-pilar role="columnheader" size="100% / 4 - 0.25rem">Name</nn-pilar>
          <nn-pilar role="columnheader" size="100% / 4 - 0.25rem">Matched</nn-pilar>
          <nn-pilar role="columnheader" size="100% / 4 - 0.25rem">Failed / Skipped</nn-pilar>
          <nn-pilar role="columnheader" size="100% / 4 - 0.25rem">Time</nn-pilar>
        </nn-fila>
...
`
		...
		#show(section) {
			...
			if (target) {
				...
				// Autofocus first input inside the section if present
		...
		#startTimer() {
			if (!this.#data.interval) {
				this.#data.interval = setInterval(() => {
					this.#data.seconds++
					if (this.#data.secondsElement) this.#data.secondsElement.innerHTML = this.#data.seconds
					if (this.#data.seconds >= 120) this.#finishGame()
				}, 1000)
			}
		}
		...
		#loadScoreboard() {
			...
			list.sort((a, b) => b.matched - a.matched || a.time - b.time)
			...
			if (list.length > 0) {
				...
				row.setAttribute('break', 'sm')
				row.setAttribute('role', 'row')
				row.innerHTML = `<nn-pilar size="100%" role="cell">No entries</nn-pilar>`
				body.appendChild(row)
			}
		}
